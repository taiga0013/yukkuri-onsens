...
マイページでアイコンをタップすると「端末の写真から選ぶ」「アプリオリジナルアイコンから選ぶ」を選択できる。
「端末の写真から選ぶ」は`expo-image-picker`で選択→`expo-image-manipulator`で最大1280px・品質80%に圧縮→
Supabase Storageの`avatars`バケット（`{user_id}/avatar.jpg`）にアップロードし、`profiles.avatar_url`を更新する。

バケット・RLSは `supabase/migrations/20260708000006_avatar_storage.sql` で作成済み（`db push`推奨、
またはSQL Editorに貼り付け）。ネイティブ専用機能のため、Web版では選択メニューは出るが実際のアップロードは
実機ビルドでのみ確認できる。
