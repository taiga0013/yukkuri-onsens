...
- **TTL自動チェックアウトの猶予を1時間に延長**（本セッションで変更）: 長風呂・サウナ休憩などでアプリを閉じている間に誤って自動チェックアウトされる問題を緩和するため、`ttl_auto_checkout`の猶予を30分→1時間に変更（`supabase/migrations/20260729000001_ttl_auto_checkout_1hour.sql`）
- **オーナー申請機能をモバイルから削除**（本セッションで変更）: 温泉地詳細画面の「オーナー申請をする」ボタン・フォームを削除。`owner_applications`テーブルと管理者ダッシュボードの承認/却下画面はそのまま残置（過去分の確認用、新規申請は来なくなる）
- **「情報を修正する」提案のAI自動判定**（本セッションで追加）: ユーザーが提案を送信すると、新しいEdge Function [evaluate-edit-suggestion](../supabase/functions/evaluate-edit-suggestion/index.ts)を呼び出す。Claude（`claude-haiku-4-5-20251001`）が現在の値と提案内容を比較し、明らかな誤りだと確信できる場合のみ`onsens`テーブルへ自動反映（`auto_approved`）、確信が持てない場合は反映せず`auto_rejected`。**`ANTHROPIC_API_KEY`のFunction Secretが未設定の間は何もせず、提案は`pending`のまま従来通り管理者の手動承認/却下に委ねられる**（フォールバック設計、キー未設定でも壊れない）。管理者ダッシュボードの「情報修正提案」画面にAI承認/却下バッジとAIの判断理由を表示するよう対応済み。**Edge Functionのデプロイ（`supabase functions deploy evaluate-edit-suggestion`）とAPIキー設定はユーザー側で未実施**
- **サブスク画面を3プラン構成に変更**（本セッションで変更）: `mobile/app/subscription.tsx`を単一プランから選択式リストに変更。湯っくりプラン（全温泉地、¥300/月）・サウナ特化プラン（サウナ施設のみ、¥300/月）・両方セット（¥500/月）の3つから選べる。引き続きUIのみのデモで実際の決済処理はなし
...
1. **`evaluate-edit-suggestion` Edge Functionのデプロイ＋`ANTHROPIC_API_KEY`設定**（このセッション環境ではCLIログイン不可のためユーザー側で未実施）。デプロイ・キー設定後に初めて「情報を修正する」のAI自動判定が動き出す
2. **iOS実機ビルド・検証**（Macが使えるようになったら。優先度は保留中との申し出あり、着手前に必要かどうか本人に確認すること）
...
3. Android実機があれば、EAS Buildで作ったAPKをインストールして同様に検証
4. 温泉地データの拡充、App Store/Google Play申請の準備

## 5. PowerPointプレゼン資料について

本セッションで、ポートフォリオ発表用のプレゼン資料を作成した。

- **成果物**: `C:\Users\taiga yamamoto\Desktop\湯っくり_プレゼン資料.pptx`（7分発表＋アプリ実演を想定。本セッション時点で9枚構成: ①問題提起（温泉が混んでいたら？の自作イラスト）／②サムネ（課題・きっかけは削除しターゲットのみに整理）／③他社サービスとの違い（比較表、見やすさ行は削除）／④チェックインの説明／⑤⑥アプリ概要／⑦技術スタック（フロントエンド・バックエンド・データベース・認証・地図・言語・使用AIの7カテゴリのロゴ付きグリッドに刷新）／⑧将来性（施設連携→掲載施設数の拡充に変更）／⑨サブスク画面のスクリーンショット貼り付け用プレースホルダー（本セッション追加、画像は未挿入でユーザーが後で貼る想定）
...
- **重要: `Desktop`フォルダがOneDrive同期対象になっており、`python-pptx`で保存した直後にOneDriveが古いバージョンを巻き戻すことが本セッションで複数回発生した**（冒頭スライドが2枚に重複する／末尾に空白スライドが増える、など）。対処法として確立した手順: (1) 編集前に`.pptx`をOneDrive同期対象外の作業用フォルダ（スクラッチパッド等）へコピー、(2) そこで全ての編集・LibreOffice変換・視覚確認を完了させる、(3) 最後に一度だけDesktopへコピーし直して`python-pptx`で`Presentation()`を開き直しスライド枚数を確認する。**Desktopの`.pptx`を直接繰り返し保存編集するのは避けること**。根本的に気になる場合は、このファイルだけOneDriveの同期対象から外すようユーザーに提案する
